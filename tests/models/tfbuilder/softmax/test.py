...
class OpChecker:
  ...
  def preRun(self):
     if self.dtype is "uint8":
        self.input = np.random.uniform(low=0, high=255, size=self.ishape).astype(self.dtype)
     else:
  ...
  def runTVM(self, useRemote=False, dumpIR=False):
    ...
    self.output_tvm = module.get_output(0, tvm.nd.empty(self.oshape, self.dtype, ctx=ctx)).asnumpy()
