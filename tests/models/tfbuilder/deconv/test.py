...
class ConvOpChecker:
  def __init__(self, name, ishape, oshape, fshape, strides=(1, 1, 1, 1), dilation=(1, 1, 1, 1), dtype="float32"):
  ...
  def genModels(self):
    ...
    with tf.Session(graph=tf.Graph()) as sess:
      data = tf.placeholder(tf.float32, shape=self.ishape, name=self.iname)
      Weights = tf.Variable(tf.truncated_normal(self.fshape, stddev=0.1), name=self.fname)

      deconv = tf.nn.conv2d_backprop_input(input_sizes=self.oshape, filter=Weights, out_backprop=data,
                                           strides=list(self.strides), padding="SAME",
                                           use_cudnn_on_gpu=False, data_format='NHWC',
                                           dilations=list(self.dilation), name=self.name)

      sess.run(tf.global_variables_initializer())
      constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def, [self.name])

      with tf.gfile.FastGFile(self.name + ".pb", mode='wb') as f:
        f.write(constant_graph.SerializeToString())

    coreml_model = tfcoreml.convert(tf_model_path=self.name + ".pb",
                                    mlmodel_path=self.name + ".mlmodel",
                                    input_name_shape_dict={self.iname + ":0" : self.ishape},
                                    output_feature_names=[self.name + ":0"])
  ...
  def runTensorFlow(self):
    ...
    with tf.Session(graph=g) as sess:
      ...
      outputs = [sess.graph.get_tensor_by_name("import/" + self.name + ":0")]
  ...
  def runTVM(self):
    ...
    with nnvm.compiler.build_config(opt_level=1):
      graph, lib, params = nnvm.compiler.build(sym, target,
              shape={self.iname + "__0" : nhwc2nchw(self.ishape)}, params=params)
    ...
    self.output_tvm = module.get_output(0, tvm.nd.empty(nhwc2nchw(self.oshape),
                                        self.dtype)).asnumpy()
...
op = ConvOpChecker("deconv", input_shape, output_shape, filter_shape, strides=strides)
op.genModels()
op.preRun()
op.runTensorFlow()
op.runTVM()
...
np.testing.assert_allclose(op.output_tvm, np.transpose(op.output_tf, axes=(0, 3, 1, 2)), atol=1e-3, rtol=1e-3)
