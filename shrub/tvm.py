...
class RuntimeWrapper:
    def __init__(self, model, target, deployables):
        if target.rpc.path:
            ...
            rparams = {k: tvm.nd.array(v, remote.context(
                str(target.target), 0)) for k, v in deployables.params.items()}
    ...
    def run(self, model, useLayout, dumpOutput=False):
        ...
        for i in range(len(model.outputs)):
            model.outputs[i].ndarray = self.module.get_output(i, tvm.nd.empty(
                model.outputs[i].shape, model.outputs[i].dtype, ctx=self.ctx)).asnumpy()
